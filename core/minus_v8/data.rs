...
use crate::minus_v8::{HandleScope, Local, Handle};
use crate::serde_v8;
use crate::serde_v8::{ErasedDeserializer, ErasedSerialize, from_backend, from_v8, to_backend};

#[derive(Clone)]
pub enum Value {
  Object,
  String { inner: StdString },
  Boolean { inner: bool },
  Integer { inner: i32 },
  Function { id: i32 },
  Promise { id: i32 },
  Exception { class: StdString, message: StdString },
  Null,
  Undefined,
}
...
impl Function {
  pub fn call<'s>(
    ...
    let serialized_args = args.iter()
      .map(|x| {
        let val: serde_v8::Value = from_v8(scope, x.clone()).unwrap();
        to_backend(scope, val).unwrap()
      })
      .collect::<Vec<_>>();
    let mut serialized_result = self.call_with_serialized(scope, recv, serialized_args);
...
impl_try_froms! {
  ...
  Value::Function { id } | Value::Integer { inner as id } => Function(id),
  Value::Promise { id } | Value::Integer { inner as id } => Promise(id),
}
